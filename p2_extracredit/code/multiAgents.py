...
def betterEvaluationFunction(currentGameState):
    ...
    if min(ghostDistance)<4: #penalize if the agent gets too close to the ghost
        return -10000000
    return currentGameState.getScore()+max(newScaredTimes) +1/closestFood #Reciprocal because if pacman is far away from the closest food, then we will penalize pacman.
    #util.raiseNotDefined()
