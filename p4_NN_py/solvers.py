...
class GradientDescentSolver(Solver):
    ...
    def get_updates_with_momentum(self, loss_tensor, param_vars):
        ...
        n=len(param_vars)
        for i in range(0,n):
            updates.append((param_vars[i],param_vars[i]-self.learning_rate*grad_tensors[i]+self.momentum*vel_vars[i]))
...
class StochasticGradientDescentSolver(GradientDescentSolver):
    ...
    def solve(self, input_train_data, target_train_data, input_val_data, target_val_data, model, callback=None):
        ...
        valGenerator = MinibatchIndefinitelyGenerator(val_data, 1, self.shuffle)
        ...
        for iter_ in range(self.iterations):
            ...
            """
            input_train, target_train = trainGenerator.next()
            train_loss = session.run(loss_tensor, feed_dict = {placeholders[0]: input_train, placeholders[1]: target_train})
            session.run(update_ops, feed_dict = {placeholders[0]: input_train, placeholders[1]: target_train})
            val_train, val_target = valGenerator.next()
            val_loss = session.run(loss_tensor, feed_dict = {placeholders[0]: val_train, placeholders[1]: val_target})
            train_losses.append(train_loss)
            val_losses.append(val_loss)
            if callback is not None: callback(model)
            self.display_progress(iter_, train_losses, val_losses)
