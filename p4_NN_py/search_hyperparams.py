...
def search_hyperparams(train_data, train_labels, val_data, val_labels,
    ...
    for model, (learning_rate, momentum, batch_size) in zip(models, hyperparams):
        ...
        Model=solvers.MinibatchStochasticGradientDescentSolver(learning_rate, iterations, batch_size, momentum)
        train_loss,val_loss=Model.solve(train_data, train_labels, val_data, val_labels, model)
        accuracy=model.accuracy(val_data, val_labels)
        if accuracy>best_accuracy:
            best_model=model
            best_accuracy=accuracy
        val_accuracies.append(accuracy)
        best_hyperparams=val_accuracies.index(max(val_accuracies))+1
